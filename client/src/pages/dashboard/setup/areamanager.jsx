import {
  ...
  export function SetupAreaManager() {
          ...
          const result = await Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              Swal.fire({
                ...
                await axios.post("/assignedManager", {
                  managerID: managerValue.value,
                  branchID: branchesValue.map((branch) => branch.value),
                });
                const results = await axios.get('/getAssignedManager');
                setInBlist(results.data);
                const response = await axios.get('/getAllBlistManager');
    ...
    useEffect(() =>{
      const fetchAreaManager = async () =>{
        try{
          const response = await axios.get('/getAllAreaManager');
    ...
    useEffect(() =>{
      const fetchInBList = async () =>{
        try{ 
          const response = await axios.get('/getAssignedManager');
  ...
  if (isConfirmed) {
    try {
      await axios.delete('/assignedDeleteManager', {
        data: {
          managerID: managerID,
      ...
      const result = await axios.get('/getAssignedManager');
